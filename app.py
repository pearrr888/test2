...
HEALTH SCORE COMPONENTS <span style="font-size:14.5px; color:#94A3B8; font-weight:normal; margin-left:6px;">องค์ประกอบที่ใช้คำนวณ Company Health Score จริง (คำนวณจากอัตราส่วนจริง)</span></div>""", unsafe_allow_html=True)

    # หมายเหตุ: 4 การ์ดนี้คือส่วนประกอบจริงของ health_score ที่คำนวณใน calculate_scores.py
    # (ROE 30% + ROA 25% + Liquidity 20% + Debt 25% = 100%) เดิมหน้านี้เคยมี "7 Dimensions"
    # (รวม Growth, Cash Flow, Efficiency, Earnings Quality) แต่ตัวเลข Weight ที่แสดงไม่ตรงกับ
    # สูตรคำนวณจริง (4 ปัจจัยนี้ไม่ได้ถูกใช้คำนวณ health_score เลย) จึงตัดออกเพื่อไม่ให้เข้าใจผิด
    rev_growth = safe(stock_info.get('revenue_growth_yoy'), 0.0)
    dim_roe = int(round(np.clip(safe(stock_info.get('roe'), 10.0) * 3.5, 0, 100)))
    dim_roa = int(round(np.clip(safe(stock_info.get('roa'), 5.0) * 7.0, 0, 100)))
    ...
    dims = [
        ("1", "📈", "ROE", "30%", dim_roe, "#10B981"),
        ("2", "📊", "ROA", "25%", dim_roa, "#3B82F6"),
        ("3", "💧", "LIQUIDITY", "20%", dim_liquidity, "#06B6D4"),
        ("4", "🛡️", "DEBT / STABILITY", "25%", dim_stability, "#EAB308"),
    ]
